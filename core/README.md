...
│  │ DelegateRequestContext│   │ SyntheticParamType   │     │
│  │   Accessor           │    │ @DisplayName         │     │
│  │ RequestContextAware  │    └──────────────────────┘     │
│  │   Executor           │                                  │
│  │ RequestContextCopying│                                  │
│  │   Decorator          │                                  │
...
兼容 .NET 的顺序 GUID 布局的 UUID 生成器，使用 .NET 纪元偏移量 `DOTNET_EPOCH_OFFSET_MILLIS = 62_135_596_800_000L`：
...
public interface ObjectPool<T> {
    int getCapacity();                       // 池容量
    int getIdleCount();                      // 空闲对象数
    int getInUseCount();                     // 使用中对象数
    ObjectPoolPolicy<T> getPolicy();         // 获取策略
    T acquire();                             // 获取对象
...
| `containsAll(Tuple)` / `containsAll(Collection<?>)` | 是否包含另一元组/集合的所有值 |
| `containsAny(Tuple)` / `containsAny(Collection<?>)` | 是否有交集 |
| `indexOf(Object)` / `lastIndexOf(Object)` | 查找位置 |
| `toArray()` / `toArray(X[])` | 转数组 |
| `equalsIgnoreOrder(Tuple)` | 忽略顺序的等值比较 |
| `iterator()` | 迭代器遍历（实现 `Iterable<Object>`） |
...
| `@Range` | `min`, `max`, `inclusiveMin`, `inclusiveMax`（默认均 false）, `message` | 数值范围检查，支持开/闭区间 |
...
`HttpStatusException` 承载 HTTP 状态码的运行时异常，子类固定状态码以支持全局异常处理映射。所有子类支持 `(String message)` 和 `(String message, Throwable cause)` 两种构造器。
...
| 类型 | 说明 |
|------|------|
| `RequestContextAccessor`（接口） | `getContext()` / `setContext()` / `removeContext()` + 静态 `get()` / `set()` / `remove()` |
| `DefaultRequestContextAccessor` | `ThreadLocal<RequestContext>` 实现 |
| `DelegateRequestContextAccessor`（`@FunctionalInterface`） | 委托访问器 |

### RequestContextAwareExecutor & RequestContextCopyingDecorator

跨线程传播 `RequestContext` 的工具：

| 类型 | 方法 | 说明 |
|------|------|------|
| `RequestContextAwareExecutor` | `fromCommonPool()` | 从 `ForkJoinPool.commonPool()` 创建 |
| | `fromExecutor()` | 从 `ThreadPoolExecutor` 创建 |
...
`SyntheticParameterizedType` 实现 `java.lang.reflect.ParameterizedType`，支持运行时动态构造泛型类型信息。`GenericType.forType(Type)` 静态工厂可基于任何现有 `Type` 创建类型令牌。
...
### Pair<K extends Comparable<K>, V> — 键值 Record
...
| 方法 | 说明 |
|------|------|
| `getMethod(Class<?>, String, Class<?>...)` | 查找 public 方法 |
...
| 方法 | 说明 |
|------|------|
| `notNull(Object, String)` | 断言非 null |
| `notNull(Object, Supplier<String>)` | 断言非 null（惰性消息） |
