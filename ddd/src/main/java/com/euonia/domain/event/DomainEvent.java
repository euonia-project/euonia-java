...
/**
 * {@link DomainEvent} 接口表示与领域模型中特定聚合相关联的事件类型。它继承自 {@link Event} 接口，继承事件的基础属性和方法，同时添加了领域事件特有的功能。
 * <p>
 * 领域事件用于捕获和表示在特定聚合上下文中发生的重要事件或变更。它们允许系统各组件之间的通信和协调，将聚合状态变更的重要信息传播给系统中可能关心这些变更的其他部分。
 * <p>
